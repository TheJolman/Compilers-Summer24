...
int main(void) {
  ...
  while (!stack.empty()) {
    ...
    if (*curr_symbol == stack.back()) {
      std::cout << "Popping nonterm on stack top and incrementing input "
                   "iterator...\n";
      ...
      print_stack(stack);
      std::cerr << "Non-matching terminal '" << *curr_symbol
                << "' found at top of stack. Terminating...\n";
      exit(1);
    }
    // const iterator to the column (nonterminal lhs) of parse table
    auto col = parse_table.find(stack.back());
    if (col != parse_table.end()) {
      if (col->second.find(*curr_symbol) ==
          col->second.end()) { // if at an error entry
        std::cerr << "No table entry for [" << stack.back() << ", "
                  << *curr_symbol << "]. Terminating...\n";
        exit(2);
      }
    } else {
      std::cerr << "Invalid lhs '" << stack.back()
                << "' found. Terminating...\n";
      exit(2);
    }
